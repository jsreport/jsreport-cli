...
exports.find = function find (cwd) {
  return new Promise((resolve, reject) => {
    ...
    const jsreportModuleInfo = getJsreportModuleInstalled(cwd, existsPackageJson)

    if (!jsreportModuleInfo) {
      return reject(new Error(
        'Couldn\'t find a jsreport installation necessary to check if the command is available, ' +
        'if the command is a valid one try to install jsreport first.'
      ))
    }
    ...
    const jsreportEntryPoint = (userPkg.jsreport || {}).entryPoint
