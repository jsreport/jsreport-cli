...
const findProcessByCWD = Promise.promisify(daemonHandler.findProcessByCWD)
...
async function saveResponse (stream, headers, output, meta) {
  ...
  return new Promise((resolve, reject) => {
    listenOutputStream(outputStream, () => {
      ...
      fs.writeFile(meta, JSON.stringify(responseHeadersToMetaPOCO(headers)), 'utf8', (err) => {
        if (err) {
...
function getOptions (argv) {
  ...
  if (argv.template) {
    renderingOpts.template = assign({}, renderingOpts.template, argv.template)
  }

  if (argv.data) {
    renderingOpts.data = assign({}, renderingOpts.data, argv.data)
  }
