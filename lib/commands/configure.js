...
exports.handler = async (argv) => {
    ...
    const questions = [
      {
        ...
        name: 'reuseSecretKey',
        message: (answers) => {
          return `Do you want to re-use secret text from existing ${localConfigPath} file?`
        },
        default: true,
        when: () => {
          if (fs.existsSync(localConfigPath)) {
            try {
              const conf = JSON.parse(fs.readFileSync(localConfigPath).toString())

              return conf.encryption != null && conf.encryption.secretKey != null && conf.encryption.secretKey.length === 16
            } catch (e) {
              return false
            }
          }
      ...
      },
      {
        type: 'input',
        name: 'secretKey',
        message: 'Define a secret text for general encryption and authentication operations:',
        default: () => {
          return nanoid(16)
        },
        validate: (input) => {
          ...
          return 'secret must be a 16 characters string'
        },
        when: (answers) => {
          if (answers.reuseSecretKey == null || answers.reuseSecretKey === false) {
            return true
          }
  ...
  if (answers.reuseSecretKey === true) {
    const previousSecretKey = JSON.parse(fs.readFileSync(localConfigPath).toString()).encryption.secretKey
  ...
  extensionsConf.scripts = { timeout: 40000 }
