...
function initializeApp (cwd, force, selectedVersion) {
  return new Promise((resolve, reject) => {
    ...
    checkOrInstallJsreport(cwd, existsPackageJson, selectedVersion, (err, jsreportPkgInfo) => {
      if (err) {
        const errorToReject = new Error('Unexpected error happened')
...
function checkOrInstallJsreport (cwd, existsPackageJson, version, cb) {
  ...
  if (!detectedJsreport) {
    ...
    install(versionToInstall, { save: true }, (installErr) => {
