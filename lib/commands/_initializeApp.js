...
function initializeApp (cwd, force, selectedVersion, customInstall) {
  return new Promise((resolve, reject) => {
    const existsPackageJson = fs.existsSync(path.join(cwd, './package.json'))
    let isMainJsreport = false

    checkOrInstallJsreport(cwd, existsPackageJson, selectedVersion, customInstall, (err, jsreportPkgInfo) => {
      if (err) {
      ...
      if ((!fs.existsSync(path.join(cwd, './jsreport.config.json')) || force) && isMainJsreport) {
        console.log('Creating default config jsreport.config.json')
...
function checkOrInstallJsreport (cwd, existsPackageJson, version, customInstall, cb) {
