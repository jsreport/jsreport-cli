...
Commander.prototype.registerCommand = function registerCommand (commandModule) {
  ...
  var commandConfiguration = commandModule.configuration || {}
  var bindedHandler
  var newBuilder
  var self = this
  ...
  if (commandConfiguration.globalOptions) {
    commandConfiguration.globalOptions = commandConfiguration.globalOptions.map(function (opt) {
    ...
    commandConfiguration.globalOptions.filter(Boolean)

    // always add some options to global
    AVAILABLE_GLOBAL_OPTIONS.alwaysGlobal.forEach(function (opt) {
      if (commandConfiguration.globalOptions.indexOf(opt) === -1) {
        commandConfiguration.globalOptions.unshift(opt)
      }
    })
  } else {
    // always add context and verbose as global options
    commandConfiguration.globalOptions = AVAILABLE_GLOBAL_OPTIONS.alwaysGlobal
  }
    ...
    newBuilder = function (yargs) {
      ...
      if (typeof modYargs.fail === 'function') {
        ...
        modYargs.fail(function (msg, err) {
          ...
          console.error('type jsreport ' + commandName + ' -h to get help about usage')
  ...
  self._cli.command(assign({}, commandModule, {
    ...
    handler: function executeCommandInCLIContext (argv) {
      ...
      if (argv.context) {
        // adding a function to allow command to decide if the process
        // should exit or not
        argv.context.disableProcessExit = function () {
          shouldExit = false
        }
      }
