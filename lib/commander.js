    ...
    if (userArgv._.length === 0) {
      ...
      getInstance(this, this._jsreportInstance, () => {}, this.cwd)
        ...
        .then((instanceResult) => {
          ...
          if (helpRequired || willShowHelpExplicetly) {
            return findAndLoadExtensionsCommands(instance, this, verboseMode)
          }
        })
        .then(() => {
          // activating CLI
...
function handleCommand (commander, commandName, args, options, cb) {
  ...
  ;(async () => {
    ...
    try {
      ...
      ) : getInstance(commander, null, log, cwd)
...
async function findAndLoadExtensionsCommands (instance, commander, verbose) {
  ...
  await registerExtensionsCommands(commander, instance.extensionsManager.extensions)
}
