...
const useCustomSocketDirectory = tempPaths.useCustomSocketDirectory
const ROOT_PATH = tempPaths.rootPath
...
class Commander extends events.EventEmitter {
  ...
  start (args) {
    ...
    startProccesing(this, logger, args).then((disabledExit) => {
      if (disabledExit !== true) {
        process.exit(0)
      }
    }).catch((err) => {
      if (err.exitDirectly) {
  ...
  async startAndWait (args) {
    ...
    try {
      ...
      if (e.exitDirectly !== true) {
        printError(e, { error: (msg) => { errorMessage = msg } })
      }
    }
...
async function startCLI (logger, commander, args, context) {
  ...
  await new Promise((resolve, reject) => {
    try {
      ...
      e.message = `An error ocurred while trying to execute a command: ${e.message}`
