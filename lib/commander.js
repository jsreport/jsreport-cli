...
Commander.prototype.start = function start (args) {
  ...
  if (needsPassword) {
    ...
    if (typeof needsPassword === 'string') {
      ...
      return handleCommand(self, args, optionsForStart, onBeforeCLIParse)
    }
...
function initInstance (commander, verbose, instance) {
  ...
  function _initInstance_ (commander, verbose, instance) {
    commander.emit('instance.initializing')

    return (
      instanceHandler.initialize(instance, verbose)
