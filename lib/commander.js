...
var Commander = module.exports = function Commander (cwd, options) {
  ...
  // this option tell us that rather than lookup for the entry point file
  // we should use this path/module id directly.
  // when creating daemonized processes this value is used to locate
  // the entry point rather than look up for it in the file system.
  // if it is a path it should be an absolute path
  self._entryPoint = opts.entryPoint
...
function handleCommand (commander, args, options, cb) {
  ...
  if (commander._entryPoint) {
    context.entryPointPath = commander._entryPoint
  }
  ...
  if (ignoreEntryPoint) {
    ...
    context.getInstance = getInstance(commander._jsreportInstance, commander._entryPoint, log)
  ...
  if (commander._jsreportInstance) {
    ...
    getInstanceAsync = getInstance(null, commander._entryPoint, log, cwd)
  }
...
function getInstance (prevInstance, entryPointPath, log, cwd) {
  var args = Array.prototype.slice.call(arguments)

  if (args.length === 3) {
    return _getInstance_.bind(undefined, prevInstance, entryPointPath, log)
  }

  return _getInstance_(prevInstance, log, cwd)

  function _getInstance_ (prevInstance, entryPointPath, log, cwd) {
    if (prevInstance) {
