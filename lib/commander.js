...
class Commander extends events.EventEmitter {
  ...
  registerCommand (commandModule) {
    ...
    this._cli.command(Object.assign({}, commandModule, {
      builder: newBuilder,
      handler: (argv) => {
        const commandConfig = this._commandsConfig[commandName]
...
function startCLI (log, commander, args, context) {
  ...
  process.nextTick(() => {
    try {
      ...
      cli.parse(args, { context: context }, (error, argv, output) => {
        if (error) {
      ...
      const error = new Error('An unexpected error ocurred while trying to execute the command:')
...
function printErrorAndExit (err, exit) {
  const errorMessages = errorUtils.getErrorMessages(err)
